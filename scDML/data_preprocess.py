...
def Normalization(adata, batch_key ="BATCH",n_high_var = 1000,hvg_list=None, 
                     ...
                     normalize_features = True,scale_value=10.0,verbose=True):
    """
    ...
    if(normalize_samples):
        if(verbose):
            print("Normalize counts per cell(sum={})".format(target_sum))
    ...
    if hvg_list is None:      
        if(verbose):
            print("Select HVG(n_top_genes=1000)")
        sc.pp.highly_variable_genes(adata,n_top_genes=n_high_var,subset=True)
    else:
        print("Select HVG from given highly variable genes list")
    ...
    if normalize_features:
        if(len(adata.obs[batch_key].value_counts())==1): # single batch
            if(verbose):
                print("Scale batch(scale_value={})".format(scale_value))
...
def dimension_reduction(adata,dim=100,verbose=True):
    """
    ...
    """
    if(verbose):
        print("Calculate PCA(n_comps={})".format(dim))
...
def init_clustering(emb,reso=3.0,cluster_method="louvain",num_cluster=50):
    ...
    if(cluster_method=="louvain"):
        ...
        print("Apply louvain clustring(resolution={}) initization".format(reso))
        sc.tl.louvain(emb,resolution=reso,key_added="init_cluster")
        print("Number of Cluster ={}".format(len(emb.obs["init_cluster"].value_counts())))
        print("clusters={}".format([i for i in range(len(emb.obs["init_cluster"].value_counts()))]))
    if(cluster_method=="leiden"):
        sc.pp.neighbors(emb,random_state=0)
        print("Apply leiden clustring(resolution={})  initization".format(reso))
        sc.tl.leiden(emb,resolution=reso,key_added="init_cluster")
        print("Number of Cluster ={}".format(len(emb.obs["init_cluster"].value_counts())))
        print("clusters={}".format([i for i in range(len(emb.obs["init_cluster"].value_counts()))]))
    if(cluster_method=="kmeans"):
        print("Apply kmeans clustring(num_cluster={}) initization".format(num_cluster))
    ...
    if(cluster_method=="minibatch-kmeans"): # this cluster method will reduce time and memory but less accuracy
        print("Apply minibatch-kmeans clustring(num_cluster={}) initization".format(num_cluster))
